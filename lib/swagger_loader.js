...
module.exports = app => {
  ...
  swagger.definitions = generateDefinitions(app, appRoot);

  debug('definitions %j', swagger.definitions);

  for (const root of appRoot) {
    // 解析文件注释，组装swagger json, 只解析目录第一层，不深入解析
    const scannerDir = path.join(root, 'controller');
    const generateSwagger = dir => {
      ...
      for (const name of names) {
        ...
        if (stat.isDirectory()) {
          ...
          const controlerDotPath = getControllerDotpath(filepath, scannerDir);
          ...
          for (let i = 1; i < blocks.length; i++) {
            ...
            if (routers) {
              ...
              const xcontrollers = blocks[i].match(/async\s(\w+)/);
              if (xcontrollers) {
                path_method['x-controller'] =
                  controlerDotPath + '.' + xcontrollers[1];
              }
...
function generate_comments_block(filePath) {
  ...
  const block_regex = /\/\*\*([\s\S]*?)\*\/(\s*(async)\s+(\w+))?/gm;
  return fileString.match(block_regex);
}

function getControllerDotpath(controllerPath, controllerRoot) {
  const filepath = controllerPath
    .replace(controllerRoot, '')
    .replace('.js', '')
  ...
  return filepath
    .split(path.sep)
    .map(s => {
      const str = s.replace(/[_-][a-z]/gi, s => s.substring(1).toUpperCase());
      let first = str[0];
...
function generate_parameters(request, routers, definitions) {
  const parameter = {};
  const ObjectType = [ 'boolean', 'integer', 'number', 'string', 'object' ];
...
const item_type = [ 'string', 'number', 'boolean', 'integer', 'array', 'object' ];

function generateDefinitions(app, appRoots) {
  const definitions = {};

  const definition_reader = (app, appRoot, directory) => {
    const requestDir = path.join(appRoot, directory);
  ...
  for (const appRoot of appRoots) {
    definition_reader(app, appRoot, 'schema');
  }
